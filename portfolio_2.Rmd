...
##### CORONA FEAR
corona_fear <-rbeta(N, 3,6) #we assume skewedness (more people are not afraid, less people are super afraid)


##### FACE MASK
face_mask <- rbern(N, 1/(1+exp(corona_fear))) # prob = A vector of likelihood of Face Mask given Corona Fear.

##### SANITIZING
sanitizing <- log(corona_fear+0.1)

#sanitizing <- rlnorm(N, meanlog=3, sdlog = corona_fear)

#sanitizing <- exp(corona_fear+2)
#sanitizing <- rlnorm(N, meanlog = (corona_fear+1.5))# Sanitizing is dependent on Corona Fear
hist(sanitizing)
...
# sanitizing and corona fear plotted
ggplot(df, aes(x=sanitizing, y=corona_fear, colour = face_mask))+
  geom_point(geom = "point", shape = 19)

#eah: one simple way is to use values from one variable as the mean in the normal distribution from which another variable is drawn. That makes them correlated. McElreath does this in the boon a few times when he simulates data :)
```


```{r}
##### GERM LEVEL
germ_level <-  rnorm(N, mean=3500 , sd= 10) # germ-level is dependent on face mask and sanitizing
  

##### INFECTED
infected <-  rbinom(N, size= 1, prob= 0.3) #probability??? # (OBS)
...
# Breakfast (80 percent of canteen-customers have had breakfast on the given day)
B <- sample(0:1, size=N, replace=TRUE, prob=c(.20,.80))
 
# Deliciousness 
D <- sample(1:5, size=N, replace=TRUE, prob=c(.05, .1, .2, .35, .3))

# Hunger (dependent on B and D)
H <- rnorm(N, mean = 6 + D - (5*B), sd = 2)

# Grumbling of stomach (dependet on H)
G <- rbinom(N, size = 1, prob = (0.05 + H*0.01))

# Amount of food on plate (dependent D, H and G)
A <- rlnorm(N, meanlog = (D + 0.3*H + 0.8*G))
