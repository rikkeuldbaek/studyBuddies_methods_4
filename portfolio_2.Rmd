...
##### CORONA FEAR
corona_fear <-rnorm(N, mean=50, sd=10)
...
##### SANITIZING
sanitizing <-  rnorm(N, mean= corona_fear-20, sd=2)


##### GERM LEVEL
#germ_level <-  rnorm(N, mean= 3500 (-sanitizing*(face_mask)), sd=2)
#germ_level <-  rnorm(N, mean=(-sanitizing*(face_mask))*0.5, sd=1000) #FM =1 ingen facemask FM=2 facemask

#germ_level <-  rnorm(N, mean= 3500 - ((sanitizing*100)* (1+face_mask)), sd=100) #giver 2 slopes for mask


df$face_mask <- (ifelse(df$face_mask == "0", 2, 1)) #2 is now NOT FM and 1 is FM
germ_level <-  rnorm(N, mean= 5000 - ((sanitizing*10)* (2+face_mask)), sd=100)# SER FUCKING RIGTIGT UD


plot(germ_level, sanitizing)
...
##### INFECTED
infected <-  rbinom(N, size= 1, prob= inv_logit(scale(germ_level))) #probability??? # (OBS)
